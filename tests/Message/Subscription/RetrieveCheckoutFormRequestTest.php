...
use Omnipay\Iyzico\Message\Subscription\RetrieveCheckoutFormRequest;
...
{
    private RetrieveCheckoutFormRequest $request;

    protected function setUp(): void
    {
        $httpClient = $this->createMock(ClientInterface::class);
        $httpRequest = $this->createMock(HttpRequest::class);
        $this->request = new RetrieveCheckoutFormRequest($httpClient, $httpRequest);
    }
    ...
    {
        $this->request->setToken('checkout_token_001');
        ...
        $this->assertSame('checkout_token_001', $data['token']);
    }
    ...
    {
        $this->request->setToken('checkout_token_002');

        $this->assertSame('checkout_token_002', $this->request->getToken());
    }

    public function testSetTokenReturnsSelf(): void
    {
        $result = $this->request->setToken('checkout_token_002');
    ...
    {
        $this->request->setToken('invalid_token');
        $this->request->setLocale('TR');
        ...
        $this->assertStringContainsString('Checkout form not found', $response->getMessage());
    }
}
