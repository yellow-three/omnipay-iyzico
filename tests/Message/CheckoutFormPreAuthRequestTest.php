...
use Omnipay\Iyzico\Message\CheckoutFormPreAuthRequest;
...
class CheckoutFormPreAuthRequestTest extends TestCase
{
    private CheckoutFormPreAuthRequest $request;
    ...
    protected function setUp(): void
    {
        ...
        $this->request = new CheckoutFormPreAuthRequest($httpClient, $httpRequest);
    }
    ...
    public function testSendDataWithFailedCheckoutReturnsFailedResponse(): void
    {
        ...
        $httpClient->expects($this->once())
            ...
            ->willReturn(json_encode([
                'status' => 'failure',
                'errorCode' => '5002',
                'errorMessage' => 'Checkout form preauth initialization failed',
        ...
        $this->assertStringContainsString('Checkout form preauth initialization failed', $response->getMessage());
    }
    ...
    public function testSendDataMapsCurrencyCorrectly(): void
    {
        ...
        $httpClient->expects($this->once())
            ->method('post')
            ->with(
                $this->stringContains('/payment/iyzipos/checkoutform/initialize/preauth/ecom'),
                $this->anything(),
                $this->anything()
            )
            ->willReturn(json_encode([
