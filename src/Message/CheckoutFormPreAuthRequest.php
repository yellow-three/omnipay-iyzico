...
class CheckoutFormPreAuthRequest extends AbstractRequest
{
    ...
    public function sendData($data): Response|RedirectResponse
    {
        ...
        $result = CheckoutFormInitializePreAuth::create($request, $options);

        $response = new RedirectResponse($this, $result);
        $response->setRedirectUrl($result->getPaymentPageUrl() ?? '');
        $response->setRedirectMethod('GET');
        $response->applySignature($this->getSecretKey(), 'checkout-preauth-init');
